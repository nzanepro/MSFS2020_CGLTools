...
def test_create_level_tile_and_sub_deltas_constant_field_roundtrips(tmp_path):
    """For a spatially-constant elevation field, downsampling (via the
    Gaussian blur + decimation in this branch) and then pyrUp'ing back
    should reproduce the same constant almost everywhere, so the Laplacian
    delta should be ~0 in the interior of the tile (away from border
    effects). This is a real correctness property of the pyramid encoding,
    ...
    for i in range(4):
        ...
        # point of this blur/decimate + pyrUp pipeline away from borders)
