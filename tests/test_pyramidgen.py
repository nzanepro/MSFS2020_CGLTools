...
def test_create_level_tile_and_sub_deltas_constant_field_roundtrips(tmp_path):
    """For a spatially-constant elevation field, downsampling and then
    pyrUp'ing back should reproduce the same constant almost everywhere, so
    the Laplacian delta should be ~0 in the interior of the tile (away from
    border effects). This is a real correctness property of the pyramid
    encoding, not just a shape check.
    """
    qkey = "023011"
    beginningcoords = bt.QuadKeyToTileXY(qkey + '0')
    value = 500
    ...
    for i in range(4):
        ...
        # point of this downsample + pyrUp pipeline away from borders)
