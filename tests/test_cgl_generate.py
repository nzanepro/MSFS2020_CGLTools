...
def test_create_blob_mt_global_state_does_not_leak_between_independent_calls(tmp_path):
    """cgl_generate keeps a module-level `compressedreturns` dict that
    createBlobMT populates by chunk-index but never clears. If a later
    createCGL() call (e.g. for a second/smaller CGL in the same process,
    as createCGLs() does in a loop) produces fewer chunks than an earlier
    call, stale entries from the earlier call can leak into the new blob.
    This test builds a "large" CGL (multiple 256-tile chunks) followed by a
    "small" one (a single chunk) in the same process and checks the second
    CGL's tile count/header are internally consistent -- if this ever
    starts failing, the leak has become observable.
    """
    def _base4(i, width):
        digits = []
        for _ in range(width):
            digits.append(str(i % 4))
            i //= 4
        return "".join(reversed(digits))

    # valid dem<6-char base qkey><subkey>.bil names, subkey a real base-4
    # string so createLayout's int(subkey, 4) parsing succeeds
    many_tiles = [_make_tile(tmp_path, f"dem023011{_base4(i, 5)}.bil", bytes([i % 256]) * 50)
                  for i in range(300)]
    dst1 = tmp_path / "big" / "dem011.cgl"
    cglg.createCGL(many_tiles, str(dst1), threads=1)

    few_tiles = [_make_tile(tmp_path, "dem0990990.bil", bytes([1]) * 50)]
    dst2 = tmp_path / "small" / "dem099.cgl"
    cglg.createCGL(few_tiles, str(dst2), threads=1)

    data2 = dst2.read_bytes()
    tilecount2 = int.from_bytes(data2[32:34], "little")
    assert tilecount2 == 1
    headerstart = data2[36]
    headerlength = int.from_bytes(data2[40:43], "little")
    blob2 = data2[headerstart + headerlength:]
    # the small CGL's compressed blob should only contain the one tile's
    # compressed bytes, not leftover chunks from the earlier 300-tile CGL
    prop = 93
    pb = math.floor(prop / (9 * 5))
    prop = prop - (pb * 9 * 5)
    lp = math.floor(prop / 9)
    ...
    filters = [{"id": lzma.FILTER_LZMA1, "preset": lzma.PRESET_DEFAULT,
                "lc": lc, "lp": lp, "pb": pb, "dict_size": 65536}]
    restored = lzma.decompress(bytes(blob2), lzma.FORMAT_RAW, None, filters)
    assert restored == bytes([1]) * 50
