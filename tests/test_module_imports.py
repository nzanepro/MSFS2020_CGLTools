...
GenDEMCGL.py in particular does all of its real work inside a `doit()`
function only called from `if __name__ == "__main__":`, so importing it
must be a no-op (module-level manifest/Options/LongLat definitions only) --
verified below.
...
def test_gendemcgl_does_not_execute_main_workflow_on_import():
    """GenDEMCGL.py's pipeline lives inside `doit()`, only called from
    `if __name__ == "__main__": doit()`. If real work leaked to module
    scope, importing the module for testing purposes would try to shell
    out to Global Mapper or block on a confirmation prompt.
    """
    import GenDEMCGL
    assert hasattr(GenDEMCGL, "Options")
    assert hasattr(GenDEMCGL, "manifest")
    assert hasattr(GenDEMCGL, "doit")
    # TopLevelQKeys is only computed inside doit(), so it must NOT exist as
    # a module attribute after a plain import.
...
def test_genheightmaps_has_a_broken_import():
    ...
    imported at all. Documented here (rather than silently skipped) so a
    future fix is visible as a test that starts failing.
    """
