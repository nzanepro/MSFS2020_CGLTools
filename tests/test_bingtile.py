...
# ---------------------------------------------------------------------------
# ListAllSubQKeys: this branch changed its `basepath` parameter to require a
# pathlib.Path (does `basepath / "Tile" / str(level)`), unlike
# main/plain/try3 which accept a plain string. Confirms this real,
# branch-specific behavioral/API change.
# ---------------------------------------------------------------------------

def test_list_all_sub_qkeys_requires_path_not_str(tmp_path):
    with pytest.raises(TypeError):
        bt.ListAllSubQKeys([["023011", 0]], 8, str(tmp_path))


def test_list_all_sub_qkeys_works_with_path(tmp_path):
    subs = bt.ListAllSubQKeys([["023011", 0]], 8, tmp_path)
    assert len(subs) == 4 ** (8 - 6)
    assert (tmp_path / "Tile" / "8").is_dir()
