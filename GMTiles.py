...
def CreateGlobalMapperScript(index, subQKeys, options):
    script = open(
        options.Basepath + "/tilescript_" + str(index).rjust(3, '0') + ".gms",
        'w')
    ...
    for subQKey in subQKeys:
        ...
        script.write(
            ...
            str(north) + ' FILENAME="' + os.path.abspath(options.Basepath) +
            '\\Tile\\' + str(level) + "\\" + "dem" + subQKey + '.bil"\n')
    script.close()
...
def GMCreateUndulatedSource(index, subQKeys, options):
    script = open(
        options.Basepath + "/undulatescript_" + str(index).rjust(3, '0') +
        ".gms", 'w')
    script.write('GLOBAL_MAPPER_SCRIPT VERSION=1.00\n')
    script.write(
    ...
    for subQKey in subQKeys:
        ...
        script.write('COMBINE_TERRAIN COMBINE_OP=ADD LAT_LON_BOUNDS=' +
                     ...
                     str(north) + '\n')
        script.write(
            'EXPORT_ELEVATION TYPE=BIL USE_UNSIGNED=NO BYTES_PER_SAMPLE=2 ' +
            'FORCE_SQUARE_PIXELS=NO ELEV_UNITS=METERS SAMPLING_METHOD=LAYER LAT_LON_BOUNDS='
            + str(west) + ',' + str(south) + ',' + str(east) + ',' +
            str(north) + ' FILENAME="' + os.path.abspath(options.Basepath) +
            '\\UndulatedSRC\\' + "dem" + subQKey[0] + '.bil"\n')
    script.close()
...
def visualize(exepath, basepath):
    process = subprocess.Popen("\"" + exepath + "\" " + basepath +
                               '\\visualize.gmw',
                               shell=True,
