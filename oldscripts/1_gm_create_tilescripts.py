...
for qket in qkeystoprocess:
    ...
    while level <= maxlevel:
        ...
        while qkey.startswith(qket):
            if itr == 0:
                script = open(
                    "tilescript_" + str(fileindex).rjust(3, '0') + ".gms", 'w')
                ...
                # )
                script.write(
                    'IMPORT FILENAME=C:\\shows\\msfs\\gitdown\\MSFS2020_CGLTools_try3\\source\\try3.gmc ELEV_UNITS=METERS ELEV_SCALE=1 VOID_ELEV=0.0\n'
                )
                # script.write(
            ...
            script.write(
                ...
                str(north) + ' FILENAME="' + os.getcwd() + '\\Tile\\' +
                str(level) + "\\" + "dem_" + qkey + '.rw"\n')
            qkey = bingtile.QuadKeyIncrement(qkey)
            itr += 1
            if itr == maxperfile - 1:
                script.close()
                itr = 0
