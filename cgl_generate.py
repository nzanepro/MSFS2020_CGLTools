...
# when moving to 12 to greater resolutions
# Yes, cgl header byte 0x1C should be 1 instead of 2,
# QuadKeys 32 bits long and what I designate in my cgl layout generation code as "levelchangevalue", should be 268435456, instead of 4096.
...
def createCGLs(TopLevelQKeys, basepath, threads):
    ...
    for qkey in TopLevelQKeys:
        if qkey[1] == False:
            result_cgl = basepath + "\\" + qkey[0][0:3] + "/dem" + qkey[0][
                3:6] + ".cgl"
            if Path(result_cgl).exists():
                counter += 1
                continue
            files = glob.glob(basepath + "\\Tile/" + str(Options.CGLLevel) +
                              "/dem" + qkey[0] + "*.bil")
            for level in range(Options.CGLLevel + 1, Options.MaxLevel + 1):
                files += glob.glob(basepath + "\\Delta/" + str(level) +
                                   "/dem" + qkey[0] + "*.bil")
            ...
            # pp(files)
            createCGL(files, result_cgl, threads)
            counter += 1
