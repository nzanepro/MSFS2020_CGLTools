...
def custom_key(pth: Path):
    return len(pth.as_posix()), pth.as_posix().lower()
...
def createLayout(tiles):
    ...
    pervlevel = 0
    for tile in tiles:
        subkey = tile.as_posix()[str(tile.as_posix()).index("dem_") + 3 +
                                 6:str(tile.as_posix()).index(".bil")]
        subval = 0
        level = len(subkey)
        if subkey == '':
            subval = 0
...
def compressChunk(chunkid, chunk):
    ...
    for tile in chunk:
        qkey = tile.as_posix()[str(tile).index("_") +
                               1:str(tile).index(".bil")]
        infile = open(tile, 'rb')
        inarr = infile.read()
        infile.close()
        uncompressedsizes.append(len(inarr))
...
def createCGL(srcpaths, dstpath: Path, threads):
    tiles = srcpaths
...
def createCGLs(TopLevelQKeys, basepath: Path, threads):
    totalcount = 0
    ...
    for qkey in TopLevelQKeys:
        if qkey[1] == False:
            print(qkey[0])
            n = f"dem_{qkey[0]}*.bil"
            files = [f for f in Path(basepath / "Tile" / "6").glob(n)]
            # files.extend([f for f in Path(basepath / "Delta").rglob(n)])

            cgl_name = basepath / qkey[0][0:3] / f"dem_{qkey[0][3:6]}.cgl"
            createCGL(files, cgl_name, threads)
            counter += 1
